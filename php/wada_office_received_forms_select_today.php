<?php
    ...
    if($mysqli){
        $stmt = $mysqli->prepare("SELECT * FROM `wadaconnectapplicationlisting` INNER JOIN `wadaconnectapplicationtype` ON wadaConnectApplicationType = wadaConnectApplicationTypeID INNER JOIN `wadamemberpersonaldetails` ON `wadaMemberID` = `wadaConnectApplicationUserID` WHERE DATE(wadaConnectApplicationWadaSentDateTime) = CURRENT_DATE() AND wadaConnectApplicationWadaSentStatus = 'Sent' AND wadaConnectApplicationStatus != 'Approved' ORDER BY wadaConnectApplicationListingID DESC;");
        $stmt->execute();
        ...
        if($result->num_rows > 0){
            ...
            echo "<tr><td colspan='7'><center>No Applications Received Today</center></td></tr>";
        }
